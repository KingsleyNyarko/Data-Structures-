...
"""
Created on Fri Feb 17 09:38:36 2023
NYARKO KINGSLEY
6943021
...
m_ABM=min(ABM)
"""
when the bending moment is m_ABM, we get the expression x**2-Lx+(L**2/6)+(2*m_ABM)/w=0
"""
#from the above
a=1
b=-L
...
"""
re1=((root_1b-root_1f)/root_1b*100)
re2=((root_2f-root_2b)/root_2f*100)
print()
print('(g) The relative errors between estimated points of contra-flexure are{0}% and {1}%'.format(re1,re2))
...
b=-L
c=(L**2//6)+(2*min_M)/w
#Using the quadratic formula the two root are;
discriminant=b**2-4*a*c
root_1=(-b-np.sqrt(discriminant))/2*a
root_2=(-b+np.sqrt(discriminant))/2*a
print()
